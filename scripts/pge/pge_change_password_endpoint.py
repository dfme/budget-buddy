...
def role_opts(system, tools=None, budget=0.20):
    return ClaudeAgentOptions(
        ...
        allowed_tools=tools or [], max_turns=15, max_budget_usd=budget,
    )

planner_o   = role_opts("Du bist Planner. Zerlege die Aufgabe in eine knappe Spec: Inputs, Outputs, Edge Cases. Kein Code.")
generator_o = role_opts("Du bist Generator. Setze die Spec exakt in Java um. NUR Code.", budget=0.30, tools=["Read", "Write"])
...
async def main():
    task = "Erstelle einen Endpoint zum Ändern des eigenen Passworts: PUT /users/me/password im File UserController.java. Implementiere nur den Rest Endpoint, den Inhalt kannst du leer lassen und wird später implementiert."
    print("=== PLANNER ===")
