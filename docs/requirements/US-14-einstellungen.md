...
**Voraussetzung ist erfüllt:** Beide Themes liegen bereits vollständig als CSS Custom Properties in [frontend/src/styles.scss](../../frontend/src/styles.scss) und werden über `data-theme` auf `<html>` umgeschaltet (FE-UI-02, ADR-11). Bislang ist das nur über den Dev-Toggle im Styleguide (`/styleguide`) sichtbar; diese Story bringt die Umschaltung zu den Nutzern.
