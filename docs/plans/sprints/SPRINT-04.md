...
Der Fix ist eine Umbenennung an drei Stellen (Zeilen 41, 43 im Query-Block). Er ist auf Zuruf noch
im selben Zug erledigt worden und liegt als `INFRA-23` im Branch `fix/INFRA-23-plans-index-pagination`
zusammen mit diesem Dokument im PR — Bereich Infrastruktur, geschätzt 1 SP. Ein Issue dazu existiert
bewusst nicht: `/plan-sprint` legt keine Issues an, die Task-ID trägt der Branch.

Warum `INFRA-23` und nicht `INFRA-21`: die Planung hatte `INFRA-21` als nächste freie ID
vorgeschlagen, weil die **Issue**-Reihe nur bis `INFRA-20` läuft. Commit `5129ffe`
(*[INFRA-21] Präsentation Zwischenstand*) hatte die ID aber schon ohne Issue verbraucht, und
`INFRA-22` ging an den Security-Review im `implement-issue`-Skill. Freie IDs müssen deshalb gegen
**Issues und Commits** geprüft werden, nicht nur gegen die Issue-Liste:

```bash
{ gh issue list --repo dfme/budget-buddy --state all --limit 300 --json title --jq '.[].title'
  git log --oneline --all; } | grep -oE '\[INFRA-[0-9]+\]' | sort -u -t- -k2 -n | tail -3
```
