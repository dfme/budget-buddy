...
| `user === null` im `onboardingGuard`? | `true` zurückgeben. | Die Entscheidung über anonyme Nutzer gehört dem `authGuard`, der in `canActivate` davor steht und dann schon einen `UrlTree` auf `/login` geliefert hat. Ein zweiter Redirect hier wäre eine zweite Wahrheit über denselben Fall. |
